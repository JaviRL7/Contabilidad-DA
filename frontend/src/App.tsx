...
function App() {
  ...
  const deleteMovimiento = async (movimientoId: number, tipo: 'ingreso' | 'gasto', itemId: number) => {
    try {
      ...
      console.log(`${tipo} eliminado:`, response.data)
  ...
  const handleDeleteMovimiento = async (movimiento: MovimientoDiario) => {
    setDeleteAction(() => async () => {
      try {
        const response = await axios.delete(`${API_BASE_URL}/movimientos/${movimiento.fecha}`)
        console.log('Movimiento completo eliminado:', response.data)
  ...
  const procesarGastosRecurrentes = async () => {
    ...
    try {
      for (const gastoRecurrente of gastosRecurrentes) {
        ...
        // Verificar si el gasto debe ser creado
  ...
  const confirmEditRecurrent = (updatedRecurrent: {etiqueta: string, monto: number, frecuencia: 'mensual' | 'semanal' | 'diario' | 'anual', diaMes?: number}) => {
    if (editingRecurrent) {
    ...
    return (
      <MonthlyBreakdown 
        ...
        onDeleteMovimiento={(movimiento) => {
          setDeleteAction(() => () => handleDeleteMovimiento(movimiento))
          setShowDeleteModal(true)
        }}
    ...
    <div className={`min-h-screen ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
      ...
      <header className={`${isDark ? 'bg-gray-800' : 'bg-white'} shadow-sm border-b ${isDark ? 'border-gray-700' : ''} py-4`}>
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center justify-between min-h-32">
                    ...
                    <>
                      <div className="grid gap-6">
                        {currentMovimientos.map((movimiento) => {
                          ...
                          // Debug para cada movimiento en el historial
                          console.log(`🔍 DEBUG - Movimiento ${movimiento.fecha}:`)
                          movimiento.gastos.forEach((gasto, index) => {
                            console.log(`  Gasto ${index + 1}:`, {
                              etiqueta: gasto.etiqueta,
                              es_recurrente: gasto.es_recurrente,
                          ...
                          console.log(`  👁️ Final - tiene gastos recurrentes:`, tieneGastosRecurrentes)
                          ...
                          <div key={movimiento.id} className={`rounded-lg shadow p-6 ${
                            ...
                            <div className="flex justify-between items-start mb-4">
                              <div className="flex flex-col">
                                ...
                                {tieneGastosRecurrentes && (
                                  <div className={`mt-1 px-2 py-1 rounded-full text-xs font-bold inline-block w-fit ${
                                    isDark 
                    ...
                    return (
                      <>
                        ...
                        <div className="flex justify-between items-center py-2 px-3">
                          <span className={`text-sm ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>Ingresos</span>
                          <span className="text-base font-semibold text-green-500">{formatEuro(totalIngresos)}</span>
                        </div>
                        ...
                        {/* Gastos */}
                        <div className="flex justify-between items-center py-2 px-3">
                          <span className={`text-sm ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>Gastos</span>
                          <span className="text-base font-semibold text-red-500">{formatEuro(totalGastos)}</span>
                        </div>
                        ...
                        <div className="pt-2 mt-3">
                          <div className="flex justify-between items-center py-2 px-3">
                            <span className={`text-sm font-medium ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>Balance</span>
                            <span className={`text-lg font-bold ${monthlyTotal >= 0 ? 'text-blue-500' : 'text-orange-500'}`}>
                ...
                <div className="space-y-2 mb-4">
                  ...
                  <div className="pt-2 mt-3">
                    <div className="flex items-center justify-between px-3 py-2">
                      ...
                      <span className={`text-lg font-bold ${yearlyTotal >= 0 ? 'text-blue-500' : 'text-orange-500'}`}>
